...
import {
  CAR_BOOKING_STEPS,
  getCarBookingStepHref,
  type CarBookingStepId,
} from "@/features/travel/booking/car-constants";
import { useTranslation } from "@/hooks/use-translation";
import type { TranslationKey } from "@/lib/preferences/translations";

const STEP_LABEL_KEYS: Record<CarBookingStepId, TranslationKey> = {
  "choose-car": "booking.step.chooseCar",
  checkout: "booking.step.checkout",
  payment: "booking.step.payment",
  confirmation: "booking.step.confirmation",
};

type CarBookingStepperProps = {
  carId: string;
  currentStep: CarBookingStepId;
};

export function CarBookingStepper({
  carId,
  currentStep,
}: CarBookingStepperProps) {
  const t = useTranslation();
  const currentIndex = CAR_BOOKING_STEPS.findIndex(
  ...
  return (
    <nav aria-label="Booking progress" className="overflow-x-auto">
      <ol className="flex min-w-[680px] items-start">
        {CAR_BOOKING_STEPS.map((step, index) => {
          ...
          return (
            <li key={step.id} className="flex items-center">
              <Link
                href={getCarBookingStepHref(carId, step.id)}
              ...
              {index < CAR_BOOKING_STEPS.length - 1 ? (
                <span
                  aria-hidden="true"
